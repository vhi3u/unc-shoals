...
@inline function param_background_depth(x)
    ...
    # 1. Coastal ramp (0-0.05km)
    h = h0 + (h1 - h0) * smooth_step(x, 25.0, 15.0)

    # 2. Shelf break (0.05-0.12km)
    h += (h2 - h1) * smooth_step(x, 85.0, 25.0)

    # 3. Shelf slope (0.12-0.62km)
    h += (h3 - h2) * smooth_step(x, 370.0, 150.0)

    # 4. Offshore ramp (0.62-0.65km)
    h += (h4 - h3) * smooth_step(x, 635.0, 50.0)
