...
@inline function param_background_depth(x; shelf_depth=-25.0, shelf_break_end=12e3, shelf_length=50e3, ramp_width=2e3)
    h0, h1 = 0.0, -4.0             # Starts at 0m, drops to -4m coastal shelf
    h2 = shelf_depth              # depth of the shelf plateau
    h3 = shelf_depth              # maintain shelf depth until offshore ramp
    h4 = -50.0
    ...
    # 1. Coastal ramp (0 to ~3km)
    h = h0 + (h1 - h0) * smooth_step(x, 1.2e3, 0.6e3)
    ...
    # 4. Offshore ramp (shelf_end to shelf_end + ~6km)
    ramp_mid = (shelf_end + (shelf_end + 6e3)) / 2.0
    h += (h4 - h3) * smooth_step(x, ramp_mid, ramp_width)
...
@inline function _param_shoal_bottom(x, y, y0, sigma, Hs, half_extent, shoal_length, shelf_depth, shelf_break_end, shelf_length, ramp_width)
    # 1. Background depth
    hw = param_background_depth(x; shelf_depth=shelf_depth, shelf_break_end=shelf_break_end, shelf_length=shelf_length, ramp_width=ramp_width)
...
function dshoal_param_bottom(Ly;
    ...
    bottom(x, y) = _param_shoal_bottom(x, y, y0, sigma, Hs, half_extent, shoal_length, shelf_depth, shelf_break_end, shelf_length, ramp_width)
    return bottom
end
