...
# Aₛ = get_south_area(grid)
# Aₙ = get_north_area(grid)
# Aₑ = get_east_area(grid)

# u_E = ((Aₛ * v_S) - (Aₙ * v_N)) / Aₑ

# integrate flow to the north (0.1) integrate flow to south (-0.05); whatever mass is missing/extra , add that to east boundary condition v_E (mass flux out of domain)
...
# convergent, bi-directional flow case

# u_bcs = FieldBoundaryConditions(;
#     south = OpenBoundaryCondition(v_S, scheme=PerturbationAdvection()),
#     north = OpenBoundaryCondition(v_N, scheme=PerturbationAdvection())
# )

# v_bcs = FieldBoundaryConditions(;
#     east = OpenBoundaryCondition(v_E, scheme = PerturbationAdvection())
# )
...
# @inline south_mask_linear(x,y,z,p) = 
#     ifelse((y₀s <= y <= y₁s),
#            (y - y₀s) / (y₁s - y₀s),
#            ifelse((y₁s <= y <= y₂s),
#                   (y - y₂s) / (y₁s - y₂s), 0.0)
...
# @inline north_mask_linear(x,y,z,p) = 
#     ifelse((y₀n <= y <= y₁n),
#            (y - y₀n) / (y₁n - y₀n),
#            ifelse((y₁n <= y <= y₂n),
#                   (y - y₂n) / (y₁n - y₂n), 0.0)
...
#     i = div(size(b, 1), 2)         # x midpoint
#     k = size(b, 3)                 # surface (top layer)

#     j_south = 1                   # y = 0
#     j_north = size(b, 2)          # y = Ly

#     b_south = b[i, j_south, k]
#     b_north = b[i, j_north, k]

#     @info "🌡️ Surface buoyancy at south (y=0): $(b_south) m/s²"
#     @info "🌡️ Surface buoyancy at north (y=Ly): $(b_north) m/s²"
# end
...
# w_zavg = Average(w, dims=(3))
# b_zavg = Average(b, dims=(3))
# uv_zavg = Average(uv, dims=(3))
# uw_zavg = Average(uw, dims=(3))
# vw_zavg = Average(vw, dims=(3))
# ub_zavg = Average(uB, dims=(3))
# vb_zavg = Average(vB, dims=(3))
# wb_zavg = Average(wB, dims=(3))

# simulation.output_writers[:zaverages] = NetCDFWriter(model, (; u_zavg, v_zavg, w_zavg, b_zavg), 
#                                                           schedule = AveragedTimeInterval(1days, window = 1days),
#                                                           filename = "timeaveragedzfields.nc",
#                                                           overwrite_existing = overwrite_existing)

# simulation.output_writers[:surface_slice_writer] = NetCDFWriter(model, (; u, v, w, b), filename = "shoal_top.nc",
#                                                                       schedule = TimeInterval(1days), indices=(:, :, 1),
#                                                                       overwrite_existing = overwrite_existing)

# simulation.output_writers[:y_slice_writer] = NetCDFWriter(model, (; u, v, w, b), filename = "shoal_midy.nc",
#                                                                 schedule = TimeInterval(1days), indices=(:, round(Ny/2), :),
#                                                                 overwrite_existing = overwrite_existing)
