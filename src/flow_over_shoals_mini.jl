...
include("dshoal_vn_mini.jl")

# simulation knobs
run_number = 2000 # <-- change this for each new run
sim_runtime = 20days
callback_interval = 86400seconds
run_tag = (periodic_y ? "periodic" : "bounded") * "_shoals_mini$(run_number)"  # e.g. "periodic_run1"

if LES
    params = (; Lx=1000, Ly=2000, Lz=50, Nx=30, Ny=30, Nz=10)
else
    params = (; Lx=1000, Ly=2000, Lz=50, Nx=30, Ny=30, Nz=10)
end
if arch == CPU()
...
if shoal_bath
    ...
    sigma = 80.0       # Gaussian width of shoal (half crossover)
    shoal_length = 200.0 # Horizontal span of the shoal ridge
...
const σ_off = 200.0   # half-width of sigmoid taper (m)
@inline offshore_mask(x, y, z, p) = 0.5 * (1.0 + tanh((x - 600.0) / σ_off))
