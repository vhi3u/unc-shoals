...
KE_c = @at (Center, Center, Center) KineticEnergy(model)

slice_fields = (; u_c, v_c, w_c, T, S, Ro, KE_c)
...
# Depth-averaged XY maps of fluxes (for offshore flux surface plots)
uT_zavg = Average(uT, dims=3)
vT_zavg = Average(vT, dims=3)
wT_zavg = Average(wT, dims=3)
uS_zavg = Average(uS, dims=3)
vS_zavg = Average(vS, dims=3)
wS_zavg = Average(wS, dims=3)
u_zavg = Average(u, dims=3)
v_zavg = Average(v, dims=3)

simulation.output_writers[:flux_xy] = NetCDFWriter(
    model,
    (; uT_zavg, vT_zavg, wT_zavg,
        uS_zavg, vS_zavg, wS_zavg,
        u_zavg, v_zavg),
    schedule=AveragedTimeInterval(output_interval, window=output_interval),
    filename="flux_xy_$(run_tag).nc",
    overwrite_existing=overwrite_existing)
...
# 3D time-averaged KE and EKE (full domain)
simulation.output_writers[:ke_3d] = NetCDFWriter(
    model,
    (; KE_avg, ∫KE, EKE_avg),
    filename="KE_3d_$(run_tag).nc",
    schedule=AveragedTimeInterval(output_interval, window=output_interval),
    overwrite_existing=overwrite_existing)

# KE at mid-y XZ slice (lighter weight, kept for quick diagnostics)
